...
const TerritoryGame = () => {
    ...
    const applyPowerUp = (type: PowerUpType, playerIdx: number) => {
        ...
        // Cleanup UI effect after time (handled by useEffect?)
        // We'll let gameLoop remove effect flags
    };
            ...
            for (let i = 0; i < newPlayers.length; i++) {
                for (let j = i + 1; j < newPlayers.length; j++) {
                    if (checkCollision(newPlayers[i], newPlayers[j])) {
                        ...
                        // We can't set state directly here reliably if loop runs many times, 
                        // but we can piggyback or use a ref. 
                        // Actually, let's just use a side-effect ref queue or similar?
                        // Or just update a particle state updater?
                        // Simplest: We are inside setPlayers updater. We can't setParticles here easily without race conditions
                        // UNLESS we move particles into the same big state object or use a ref for particles to pushing.
                        // For visuals, a Ref is fine.
